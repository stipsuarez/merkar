      ...
      child: Consumer<StatisticsViewModel>(
        builder: (context, model, child)=>DefaultTabController(
          ...
          child: Scaffold(
            ...
            body:TabBarView(
              children: [
                (viewModel.list!=null && viewModel.list!.isNotEmpty)? SingleChildScrollView(
                  child: PieChartView(colorList: viewModel.colorList, dataMap:viewModel.createDataMapForLists(0),
                      total: viewModel.total),
                ):Center(child: Text("No hay datos")),
                (viewModel.list!=null && viewModel.list!.isNotEmpty)? SingleChildScrollView(
                  child: PieChartView(colorList: viewModel.colorList, dataMap:viewModel.createDataMapForLists(1),
                      total: viewModel.total),
                ):Center(child: Text("No hay datos")),
                (viewModel.list!=null && viewModel.list!.isNotEmpty)? SingleChildScrollView(
                  child: PieChartView(colorList: viewModel.colorList, dataMap:viewModel.createDataMapForLists(2),
                      total: viewModel.total),
                ):Center(child: Text("No hay datos")),
                (viewModel.list!=null && viewModel.list!.isNotEmpty)? SingleChildScrollView(
                  child: PieChartView(colorList: viewModel.colorList, dataMap:viewModel.createDataMapForLists(3),
                      total: viewModel.total),
                ):Center(child: Text("No hay datos")),
              ],
            ),
